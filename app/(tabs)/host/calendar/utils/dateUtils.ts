...
// Detailed booking interface matching the real API response
...
// Simplified booking interface for calendar display
 ...
 */
export function isDateInBooking(date: Date, booking: BookingData): boolean {
...
/**
 * Convert detailed booking data to simplified format for calendar display
 */
export function adaptBookingData(
  detailedBooking: DetailedBookingData
): BookingData {
  const checkInDate = new Date(detailedBooking.checkIn);
  const checkOutDate = new Date(detailedBooking.checkOut);
  const now = new Date();
  ...
  return {
    ...
    guestAvatar: undefined, // Avatar not provided in current data structure
    status,
...
/**
 * Convert array of detailed bookings to simplified format
 */
