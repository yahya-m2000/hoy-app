/**
 * Booking Confirmation Modal for the Hoy application
...
import type { BookingFormData, BookingStep } from "@modules/booking/types";
import {
  parseInitialDates,
  calculateNights as calculateNightsBetweenDates,
  formatBookingData,
  createMockBooking,
} from "@modules/booking/utils";
...
function BookingConfirmationModal() {
  ...
  const property: PropertyType = params.property ? JSON.parse(params.property as string) : null;
  const unit = params.unit ? JSON.parse(params.unit as string) : null;
  
  // Parse initial dates from route parameters
  const { startDate: initialStartDate, endDate: initialEndDate, hasInitialDates } = parseInitialDates(params);
  ...
  const [currentStep, setCurrentStep] = useState<BookingStep>(hasInitialDates ? 'guests' : 'dates');
  ...
  }, [formData.startDate, formData.endDate, property?._id, selectDatesForProperty]);
  ...
  useEffect(() => {
    if (formData.startDate && formData.endDate && !hasInitialDates) {
      checkAvailabilityAndPrice();
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [formData.startDate, formData.endDate, formData.guests.adults, formData.guests.children]);
  ...
  const checkAvailabilityAndPrice = async () => {
    ...
    try {
      ...
      const availabilityResult = await bookingService.checkAvailability(availabilityParams);
      ...
      if (!hasInitialDates) {
        ...
        setIsAvailable(true);
        const nights = calculateNightsBetweenDates(formData.startDate, formData.endDate);
        setPriceDetails({
  ...
  const handleGuestChange = (guestType: keyof BookingFormData['guests'], value: number) => {
    setFormData(prev => ({
      ...prev,
      guests: {
        ...prev.guests,
  ...
  const handleBookingConfirmation = async () => {
    ...
    if (formData.paymentMethod?.type === 'zaad') {
      try {
            ...
            .catch((err) => {
              ...
              Linking.openURL('tel:')
                ...
                .catch(() => {
                  showToast({
                    type: 'error',
                    message: 'Could not open phone dialer. Your booking was created, but you\'ll need to make the ZAAD payment manually.',
                  });
                  proceedWithBookingCreation('pending');
                });
            });
        ...
        showToast({
          type: 'warning',
          message: 'Could not launch phone dialer, but your booking was created. Please make the ZAAD payment manually.',
        });
        proceedWithBookingCreation('pending');
      }
      return;
    ...
    try {
      ...
      if (error.response) {
        if (error.response.status === 404) {
          ...
          errorMessage = error.response.data?.message || 'Invalid reservation details';
  ...
  const proceedWithBookingCreation = async (paymentStatus = 'confirmed') => {
    ...
    try {
      ...
      if (formData.paymentMethod?.id?.includes('test')) {
        ...
        result = createMockBooking(formData, property, unit, formData.paymentMethod, bookingData);
      } else {
        throw bookingError;
      }
    }

    const successMessage = formData.paymentMethod?.type === 'zaad'
    ...
    if (formData.paymentMethod?.type === 'zaad') {
      router.replace('/(stack)/bookings/confirmation?booking=' + bookingParam);
    } else {
      router.push('/(stack)/bookings/confirmation?booking=' + bookingParam);
