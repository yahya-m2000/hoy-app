...
import { HostPolicyService } from "../services";
import {
  HostPolicies,
...
export const useHostPolicies = () => {
  ...
  const fetchPolicies = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await HostPolicyService.getHostPolicies();
      setPolicies(data);
    } catch (err) {
  ...
  const updatePolicies = useCallback(
    async (updates: HostPoliciesUpdateRequest) => {
      ...
      setError(null);
      try {
        const updatedPolicies = await HostPolicyService.updateHostPolicies(
          updates
        );
...
export const useHostSetup = () => {
  ...
  const checkSetupStatus = useCallback(async () => {
    ...
    setError(null);
    try {
      const status = await HostPolicyService.checkSetupStatus();
  ...
  const setupPolicies = useCallback(async (data: HostPoliciesSetupRequest) => {
    ...
    setError(null);
    try {
      const policies = await HostPolicyService.setupHostPolicies(data);
      setSetupStatus({
  ...
  const getDefaultPolicies = useCallback(async () => {
    ...
    setError(null);
    try {
      const defaults = await HostPolicyService.getDefaultPolicies();
      return defaults;
    } catch (err) {
...
export const useHostSetupForm = (initialData?: Partial<SetupFormData>) => {
  ...
  const validateForm = useCallback(() => {
    ...
    const validation = HostPolicyService.validateSetupData(formData);
  ...
  const createSetupRequest = useCallback(() => {
    return HostPolicyService.createSetupRequest(formData);
  }, [formData]);
...
export const useRefundCalculation = () => {
  ...
  const calculateRefund = useCallback(
    async (bookingId: string, totalAmount: number) => {
      ...
      setError(null);
      try {
        const result = await HostPolicyService.calculateRefund(
          bookingId,
          totalAmount
        );
