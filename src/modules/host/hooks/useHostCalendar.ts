...
} from "../services/hostCalendarService";
import { CalendarUpdateRequest, CalendarSettings } from "../types/calendar";
...
export const usePropertyCalendar = (
  ...
  return useQuery({
    ...
    queryFn: () => fetchPropertyCalendar(propertyId, startDate, endDate),
    enabled: !!propertyId,
...
export const usePropertyBookings = (
  ...
  return useQuery({
    ...
    queryFn: () => fetchPropertyBookings(propertyId, startDate, endDate),
    enabled: !!propertyId,
...
export const useCalendarMutations = (propertyId: string) => {
  ...
  const updateCalendarMutation = useMutation({
    mutationFn: (updateRequest: CalendarUpdateRequest) =>
      updatePropertyCalendar(updateRequest),
    onSuccess: () => {
      invalidateCalendar();
    },
    onError: (error) => {
  ...
  const updateAvailabilityMutation = useMutation({
    mutationFn: ({
      ...
      endDate: string;
      isAvailable: boolean;
    }) => updateAvailability(propertyId, startDate, endDate, isAvailable),
    onSuccess: () => {
      invalidateCalendar();
    },
  });
  ...
  const updatePricingMutation = useMutation({
    ...
    }) => updatePricing(propertyId, dates, price, promotionPercentage),
    onSuccess: () => {
      invalidateCalendar();
    },
  });
  ...
  const updateBlockedDatesMutation = useMutation({
    mutationFn: ({
      ...
      isBlocked: boolean;
      blockReason?: string;
    }) => updateBlockedDates(propertyId, dates, isBlocked, blockReason),
    onSuccess: () => {
      invalidateCalendar();
    },
  });
  ...
  const updateSettingsMutation = useMutation({
    mutationFn: (settings: Partial<CalendarSettings>) =>
      updateCalendarSettings(propertyId, settings),
    onSuccess: () => {
      invalidateCalendar();
    },
  });
