...
import { Text, Container, Icon } from "@shared/components/base";
import { Calendar } from "@shared/components/common/Calendar";
import { useTheme } from "@shared/hooks/useTheme";
import { useCalendar } from "@shared/context/CalendarContext";
...
export const DateStep: React.FC<DateStepProps> = ({
  ...
  useEffect(() => {
    if (calendarContext && isValidPropertyId) {
      // Force refresh the booked dates for this property to ensure they're up-to-date
      calendarContext.actions.refreshBookedDates(propertyId!);

      // Also invalidate any cached availability data
      calendarContext.actions.invalidateCache(propertyId!);
  ...
  useEffect(() => {
    if (calendarContext && startDate && endDate) {
      ...
      if (
        startDate !== calendarContext.state.selectedDates.start ||
        endDate !== calendarContext.state.selectedDates.end
      ) {
        calendarContext.actions.setSelectedDates(startDate, endDate);
      }

      // Check availability for the selected dates
      if (isValidPropertyId) {
        calendarContext.actions.checkDateAvailability(propertyId!, unitId);
      }
    }
  }, [
    startDate,
  ...
  useEffect(() => {
    if (onAvailabilityChange && calendarContext) {
      const { isCheckingAvailability, isAvailable } = calendarContext.state;

      if (isCheckingAvailability) {
        onAvailabilityChange(null, true);
      } else {
        onAvailabilityChange(isAvailable, false);
      }
    }
  ...
  return (
    <Container style={{ flex: 1 }}>
      ...
      {(isCheckingAvailability ||
        calendarContext?.state.isCheckingAvailability) && (
        <Container
          flexDirection="row"
          alignItems="center"
          justifyContent="center"
      ...
      {!(
        isCheckingAvailability || calendarContext?.state.isCheckingAvailability
      ) &&
        (isAvailable === false ||
          calendarContext?.state.isAvailable === false) && (
          <Container
            flexDirection="row"
            alignItems="center"
            justifyContent="center"
