import { PROPERTY_ENDPOINTS } from "@shared/constants/api";
import api from "@shared/services/core/client";
import { PropertyType } from "@shared/types/property";
import { isNetworkError, logErrorWithContext } from "@shared/utils/error";
import { addToRetryQueue } from "@shared/utils/network";
...
export const searchProperties = async (
  ...
  try {
    ...
    */ // Separate handling for city, state, and country if provided directly
    ...
    // // Handle sort parameter - convert from object format if needed
    // if (query.sort) {
    //   // If sort is already a string (like "rating_desc"), use it directly
    //   if (typeof query.sort === "string") {
    //     // Ensure format is field_order not field:order
    //     validParams.sort = query.sort.replace(":", "_");
    //   }
    //   // If sort is an object, convert to string with underscore format
    //   else if (
    //     typeof query.sort === "object" &&
    //     query.sort.field &&
    //     query.sort.order
    //   ) {
    //     validParams.sort = `${query.sort.field}_${query.sort.order}`;
    //   }
    //   // Pass through as is if it's something else
    //   else {
    //     validParams.sort = query.sort;
    //   }
    // }
