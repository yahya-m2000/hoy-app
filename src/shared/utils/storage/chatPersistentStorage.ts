...
import { ChatConversation, ChatMessage } from "@shared/types";
...
> => {
  try {
    ...
    if (Date.now() - parsedData.timestamp > CACHE_EXPIRY) {
      safeLogger.info("Conversation cache expired");
      return null;
    }
...
export const loadConversation = async (
  ...
  try {
    ...
    // Check if data is expired
    if (Date.now() - parsedData.timestamp > CACHE_EXPIRY) {
      safeLogger.info(`Conversation ${conversationId} cache expired`);
      return null;
    }
...
export const saveUnreadCount = async (count: number): Promise<void> => {
  try {
    ...
    safeLogger.debug(`Saved unread count (${count}) to persistent storage`);
  } catch (error) {
    safeLogger.error("Error saving unread count to persistent storage:", error);
  }
};
...
export const loadUnreadCount = async (): Promise<number | null> => {
  try {
    ...
    // Check if data is expired
    if (Date.now() - parsedData.timestamp > CACHE_EXPIRY) {
      safeLogger.info("Unread count cache expired");
      return null;
    }
...
export const clearChatStorage = async (): Promise<void> => {
  try {
    ...
    if (chatKeys.length > 0) {
      ...
      safeLogger.info(`Cleared ${chatKeys.length} chat storage items`);
    }
  } catch (error) {
    safeLogger.error("Error clearing chat storage:", error);
  }
};
...
export const cleanupExpiredCache = async (): Promise<void> => {
  try {
    ...
    safeLogger.error("Error cleaning up expired cache:", error);
  }
};
