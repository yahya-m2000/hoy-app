...
import { Text, Input, Button, TimeInput } from "@shared/components";
import { spacing } from "@core/design";
import type { HostHouseRules } from "@core/types";

interface HouseRulesFormProps {
  data: Partial<HostHouseRules>;
  errors: Record<string, string>;
  onChange: (data: Partial<HostHouseRules>) => void;
}
...
export const HouseRulesForm: React.FC<HouseRulesFormProps> = ({
  ...
  const handleInputChange = (field: keyof HostHouseRules, value: any) => {
    onChange({
      ...data,
      [field]: value,
    });
  ...
  const handleSwitchChange = (field: keyof HostHouseRules, value: boolean) => {
    onChange({
      ...data,
      [field]: value,
    });
  ...
  const removeRule = (index: number) => {
    ...
    onChange({
      ...data,
      additionalRules: currentRules.filter(
          ...
          <View style={styles.section}>
            ...
            {data.additionalRules && data.additionalRules.length > 0 && (
              <View style={styles.rulesList}>
                {data.additionalRules.map((rule: string, index: number) => (
                  <View
                    key={index}
